CREATE VIEW [dbo].[vwSPOStats-Employee-Syncing]
AS
SELECT 
...
--	  ,convert(decimal(18,6), count(DISTINCT gsaud.UserPrincipalName))/convert(decimal(18,6), odss.TotalContractorUsers) as TotalContractorUsersPerct
      ,sum(gsaud.SyncedFileCount) as TotalSyncedFileCount
      ,eba.UserIsContractor
FROM [dbo].[GraphSharePointActivityUserDetail] gsaud
...
WHERE eba.IsDeactivated = 0 and eba.UserEnabled = 1 and eba.UserIsContractor = 0 and gsaud.SyncedFileCount > 0
GROUP BY 
