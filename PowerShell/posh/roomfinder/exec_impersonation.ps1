...
BEGIN
{
    ...
    $Directories = Set-EPAOutputDirectories -outputDirectory $outputDirectory -scriptDirectory $scriptDirectory `
        -configFile $configFile -logPrefixFileName "log-impersonationscript" -formattedDateRun $formattedDateRun -Verbose:$VerbosePreference
...
PROCESS
{
    ...
    try
    {
        ...
        $roomacct = $configkeys.RoomFinderSvcAccount
        ...
        if ($null -ne $ConnectingUserId)
        {
            ...
            Write-EPALogMessage -LogFile $LogFile -Msg ("EXO: Impersonation using ID:{0}" -f $ConnectingUserId) -Verbose:$VerbosePreference

            # Run the scriptlet
            Enable-EPAImpersonation -CmdLetLogFile $CmdLetLogFile -roomacct $roomacct
        } else
        {
