...
PROCESS {
    ...
    $csvFile = Join-Path -Path $logDirectory -ChildPath "schedule-document-metadata.csv"
    & { invoke-expression (".\epa-console.exe scanEPADocumentMetadata --onedrive-csv {0} -l {1} -v" -f $csvFile, $logFile) }
