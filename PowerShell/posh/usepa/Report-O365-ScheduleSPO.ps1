[cmdletbinding()]
<#
	Pulls sharepoint data and stores it in the database
#>
param(
...
PROCESS {
    ...
    # Run all SharePoint Reporting
    & { invoke-expression (".\epa-console.exe reportusage -r SharePoint -p D90 -l {0}" -f $logFile) }
    & { invoke-expression (".\epa-console.exe reportusage -r SharePoint -details -l {0}" -f $logFile) }
