...
PROCESS {
    ...
    $url = "https://usepa.sharepoint.com/sites/oei_development"
    # Query external users who have expired
    & { invoke-expression (".\epa-console.exe scanEPAExternalRecertifyUsers --process-option RemoveExpired --site-url {0} -l {1} -v" -f $url, $logFile) }
}
END {
